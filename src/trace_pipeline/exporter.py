    ...
    *,
    ...
    try:
        ...
        captures = source_db.execute(
            ...
            "WHERE s.state IN ('sealed','archived') ORDER BY c.segment_id,c.offset,c.capture_id"
        )
        ...
        try:
            for row in captures:
                ...
                while remaining:
                    ...
                    payload = zlib.compress(chunk, compression_level)
                    out.execute(
                        "INSERT INTO interaction_chunks VALUES(?,?,?,?,?,?)",
                        (record_id, chunk_index, f"zlib-{compression_level}", len(chunk), len(payload), sqlite3.Binary(payload)),
                    )
                    remaining -= len(chunk)
                    chunk_index += 1
                    record_stored += len(payload)
                ...
                out.execute("UPDATE interactions SET stored_bytes=? WHERE record_id=?", (record_stored, record_id))
                count += 1
                total_raw += raw_bytes
                total_stored += record_stored
                if count % 1000 == 0:
                    out.commit()
            ...
            for handle in handles.values():
                handle.close()
        expected = int(source_db.execute(
            "SELECT count(*) FROM captures c JOIN segments s USING(segment_id) WHERE s.state IN ('sealed','archived')"
        ).fetchone()[0])
        checks = {
